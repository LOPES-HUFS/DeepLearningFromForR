...
[1,] 3.788493e-07
```
보시다 싶이 첫번째 인덱스를 약 0.9983%확률로 표기합니다. 이 말은 모델이 숫자 0을 정답을 판단한 확률이 99%란 의미로 다시말해 모델이 이미지 데이터를 보고 숫자를 맞춘 것입니다. 
...
앞으로 신경망 모델의 효율성과 정확도를 좀 더 높일 수 있는 초기값 설정을 비롯한 드롭아웃 같은 다양한 방법들에 대해서 알아볼 것이고, 합성곱 신경망까지도 R 코드로 구현할 예정입니다. 
