...
앞의 숫자들은 순서대로 이 이미지가 0일 확률, 이 이미지는 1일 확률, 이미지는 2일 ... 마지막으 9일 확률을 알려주고 있는 것입니다. 참고로 R은 다른 프로그램처럼 0부터 세지 않고 1부터 셉니다. 현재 우리가 학습시킨 모델은 첫번째 인덱스를, 다시 말해 0일 확률이 약 0.9983%라고 합니다. 이 말은 모델이 숫자 0을 정답으로 판단한 확률이 99%란 의미로 다시 말해 모델이 이미지 데이터를 보고 숫자를 맞춘 것입니다.
